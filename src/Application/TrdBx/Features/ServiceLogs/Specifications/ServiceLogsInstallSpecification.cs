...
/// </summary>
public class ServiceLogsInstallSpecification : Specification<ServiceLog>
{
    public ServiceLogsInstallSpecification(int[] Ids)
    {
        Query.Include(x=>x.Subscriptions)
             .Where(x => Ids.Contains(x.CustomerId))
             .Where(x => x.ServiceTask.Equals(ServiceTask.Install))
