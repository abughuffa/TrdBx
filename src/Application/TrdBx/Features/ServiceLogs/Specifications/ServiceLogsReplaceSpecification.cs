...
/// </summary>
public class ServiceLogsReplaceSpecification : Specification<ServiceLog>
{
    public ServiceLogsReplaceSpecification(int[] Ids)
    {
        Query.Include(x => x.Subscriptions)
             .Where(x => Ids.Contains(x.CustomerId))
             .Where(x => x.ServiceTask.Equals(ServiceTask.Replace))
