...

{
    ...
    //     _context = context;
    // }
    public async ValueTask<Result> Handle(DeleteServiceLogCommand request, CancellationToken cancellationToken)
    {

        await using var context = await _dbContextFactory.CreateAsync(cancellationToken);
        var items = await context.ServiceLogs.Where(x => request.Id.Contains(x.Id)).ToListAsync(cancellationToken);
