         ...
         IRequestHandler<ServiceLogsWithPaginationQuery, PaginatedData<ServiceLogDto>>
{
    ...
    public async ValueTask<PaginatedData<ServiceLogDto>> Handle(ServiceLogsWithPaginationQuery request, CancellationToken cancellationToken)
    {
        ...
        if (request.CustomerId.HasValue.Equals(0) || request.CustomerId is null)
        {
            ...
            var data = await _context.ServiceLogs.Include(s => s.Customer).Include(s => s.Subscriptions).Include(s => s.WialonTasks).OrderBy($"{request.OrderBy} {request.SortDirection}")
                                               .ProjectToPaginatedDataAsync<ServiceLog, ServiceLogDto>(request.Specification,request.PageNumber,request.PageSize,  _typeAdapterConfig,cancellationToken);
            return data;
        }
        else
        {
