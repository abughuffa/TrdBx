...
#nullable disable warnings
    public async ValueTask<Result<byte[]>> Handle(ExportServiceLogsQuery request, CancellationToken cancellationToken)
        {
        ...
        var data = await context.ServiceLogs.ApplySpecification(request.Specification)
    .OrderBy($"{request.OrderBy} {request.SortDirection}")
...
{_localizer[_dto.GetMemberDisplayName(x=>x.ServiceNo)],item => item.ServiceNo},
...
{_localizer[_dto.GetMemberDisplayName(x=>x.SerDate)],item => item.SerDate},
                {_localizer[_dto.GetMemberDisplayName(x=>x.IsDeserved)],item => item.IsDeserved},
                 {_localizer[_dto.GetMemberDisplayName(x=>x.IsBilled)],item => item.IsBilled},
