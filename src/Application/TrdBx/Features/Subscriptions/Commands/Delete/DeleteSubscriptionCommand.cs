...

{
    ...
    //     _context = context;
    // }
    public async ValueTask<Result> Handle(DeleteSubscriptionCommand request, CancellationToken cancellationToken)
    {
        await using var context = await _dbContextFactory.CreateAsync(cancellationToken);
        var items = await context.Subscriptions.Where(x => request.Id.Contains(x.Id)).ToListAsync(cancellationToken);
