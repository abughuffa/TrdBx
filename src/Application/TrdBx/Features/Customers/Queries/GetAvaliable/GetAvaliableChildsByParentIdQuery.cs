     ...
     IRequestHandler<GetAvaliableChildsByParentIdQuery, IEnumerable<CustomerDto>>
{
    ...
    public async Task<IEnumerable<CustomerDto>> Handle(GetAvaliableChildsByParentIdQuery request, CancellationToken cancellationToken)
    {
        ...
        var customer = await db.Customers.Where(c => c.Id == request.Id).FirstOrDefaultAsync();

        if (customer is null)
        {
            var data = await db.Customers.ApplySpecification(new AvaliableChildsByParentIdSpecification(null))
        ...
        {
            var data = await db.Customers.ApplySpecification(new AvaliableChildsByParentIdSpecification(customer.ParentId))
                                          .ProjectTo<CustomerDto>(_mapper.ConfigurationProvider)
                                          .ToListAsync(cancellationToken);
            return data;
        }
