using CleanArchitecture.Blazor.Application.Features.Summaries.Invoices.Caching;
using CleanArchitecture.Blazor.Application.Features.Summaries.Invoices.DTOs;
using CleanArchitecture.Blazor.Application.Features.Summaries.SimCards.DTOs;
using CleanArchitecture.Blazor.Domain.Enums;
using DocumentFormat.OpenXml.InkML;


namespace CleanArchitecture.Blazor.Application.Features.Summaries.Invoices.Queries.GetSummary;
    ...
    public async Task<Result<InvoiceSummaryDto>> Handle(GetInvoiceSummaryQuery request, CancellationToken cancellationToken)
    {
        ...
        var data = await _context.Invoices
                    ...
                    .Select(g => new
                    {
                        ...
                        Counts = g.Count()
                    }).ProjectTo<InvoiceSummaryDto>(_mapper.ConfigurationProvider).FirstOrDefaultAsync(cancellationToken);
        if (data != null)
