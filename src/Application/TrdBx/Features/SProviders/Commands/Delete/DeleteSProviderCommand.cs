...
public class DeleteSProviderCommand:  ICacheInvalidatorRequest<Result>
{
  public int[] Id {  get; }
...
public class DeleteSProviderCommandHandler : 
             IRequestHandler<DeleteSProviderCommand, Result>

{
    ...
    //     _context = context;
    // }
    public async ValueTask<Result> Handle(DeleteSProviderCommand request, CancellationToken cancellationToken)
    {
        await using var context = await _dbContextFactory.CreateAsync(cancellationToken);
        var items = await context.SProviders.Where(x => request.Id.Contains(x.Id)).ToListAsync(cancellationToken);
