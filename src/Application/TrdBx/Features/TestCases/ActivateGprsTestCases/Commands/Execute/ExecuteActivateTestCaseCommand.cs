...
public class ExecuteActivateGprsTestCaseCommand : ICacheInvalidatorRequest<Result>
{
    public int[] Id { get; }
...
public class ExecuteActivateGprsTestCaseCommandHandler :
             IRequestHandler<ExecuteActivateGprsTestCaseCommand, Result>

{
    ...
    public async Task<Result> Handle(ExecuteActivateGprsTestCaseCommand request, CancellationToken cancellationToken)
    {
        await using var db = await _dbContextFactory.CreateAsync(cancellationToken);

        var items = await db.ActivateGprsTestCases.Where(x => request.Id.Contains(x.Id))
