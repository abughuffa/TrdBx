...
public class TicketAdvancedSpecification : Specification<Ticket>
{
    public TicketAdvancedSpecification(TicketAdvancedFilter filter)
    {
        ...
        Query.Where(q => q.TicketNo != null)
             ...
             .Where(x => x.TicketStatus == filter.TicketStatus, filter.TicketStatus is not null)
             .Where(q => q.CreatedById == filter.CurrentUser.UserId, filter.ListView == TicketListView.My && filter.CurrentUser is not null)
             .Where(x => x.CreatedAt >= todayrange.Start && x.CreatedAt < todayrange.End.AddDays(1), filter.ListView == TicketListView.TODAY)
             .Where(x => x.CreatedAt >= last30daysrange.Start, filter.ListView == TicketListView.LAST_30_DAYS);
