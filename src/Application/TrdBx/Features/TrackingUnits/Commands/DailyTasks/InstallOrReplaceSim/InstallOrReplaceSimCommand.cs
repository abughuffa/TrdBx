...
public class InstallOrReplaceSimCommandHandler :PriceSharedLogic, IRequestHandler<InstallOrReplaceSimCommand, Result<int>>
{
    ...
    private readonly IStringLocalizer<InstallOrReplaceSimCommandHandler> _localizer;
    private readonly IWialonService _wialonService;
    public InstallOrReplaceSimCommandHandler(IApplicationDbContext context,
                                         IStringLocalizer<InstallOrReplaceSimCommandHandler> localizer,
    ...
    public async Task<Result<int>> Handle(InstallOrReplaceSimCommand request, CancellationToken cancellationToken)
    {

        ////await using var _context = await _dbContextFactory.CreateAsync(cancellationToken);

        var nsim = await _context.SimCards.Where(x => x.Id == request.SimCardId).FirstAsync() ?? throw new NotFoundException($"SimCard with id: [{request.SimCardId}] not found.");
        ...
        {
            ServiceNo = await GenSerialNo(_context, "ServiceLog", request.TsDate),
        ...
        if (unit.SimCardId == null)
        {
            ...
            serviceLog.Amount = await GetSPrice(_context, ServiceTask.InstallSimCard);
        }
        else
        {
            //ReplaceSim  
            var osim = await _context.SimCards.Where(x => x.Id == unit.SimCardId).FirstAsync() ?? throw new NotFoundException($"SimCard with id: [{request.SimCardId}] not found.");
            ...
            serviceLog.Amount = await GetSPrice(_context, ServiceTask.ReplacSimCard);
        }
