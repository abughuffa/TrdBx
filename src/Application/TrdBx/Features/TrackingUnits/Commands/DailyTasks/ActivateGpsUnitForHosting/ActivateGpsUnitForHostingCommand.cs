...
public class ActivateTrackingUnitForHostingCommand : ICacheInvalidatorRequest<Result>
{
...
public class ActivateTrackingUnitForHostingCommandHandler : SubscriptionSharedLogic, IRequestHandler<ActivateTrackingUnitForHostingCommand, Result>
{
    ...
    private readonly IStringLocalizer<ActivateTrackingUnitForHostingCommandHandler> _localizer;
    private readonly IWialonService _wialonService;
    public ActivateTrackingUnitForHostingCommandHandler(IApplicationDbContext context,
                                         IStringLocalizer<ActivateTrackingUnitForHostingCommandHandler> localizer,
                                         IWialonService wialonService)
    {
        _context = context;
    ...
    public async Task<Result> Handle(ActivateTrackingUnitForHostingCommand request, CancellationToken cancellationToken)
    {
        //await using var _context = await _dbContextFactory.CreateAsync(cancellationToken);

        var unit = await _context.TrackingUnits.Where(x => x.Id == request.Id).Include(x => x.Subscriptions).ThenInclude(s => s.ServiceLog).FirstAsync() ?? throw new NotFoundException($"TrackingUnit with id: [{request.Id}] not found.");
