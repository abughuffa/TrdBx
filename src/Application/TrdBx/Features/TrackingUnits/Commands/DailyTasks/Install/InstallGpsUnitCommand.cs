...
public class InstallTrackingUnitCommandHandler : SubscriptionSharedLogic, IRequestHandler<InstallTrackingUnitCommand, Result<int>>
{
    ...
    public InstallTrackingUnitCommandHandler(
       IApplicationDbContextFactory dbContextFactory,IWialonService wialonService)
    {
       _dbContextFactory = dbContextFactory;
       _wialonService = wialonService;
    }
    public async ValueTask<Result<int>> Handle(InstallTrackingUnitCommand request, CancellationToken cancellationToken)
    {
        await using var context = await _dbContextFactory.CreateAsync(cancellationToken);

        var unit = await context.TrackingUnits.Include(u => u.Subscriptions).ThenInclude(s => s.ServiceLog).Where(x => x.Id == request.Id).FirstAsync() ?? throw new NotFoundException($"TrackingUnit with id: [{request.Id}] not found.");
        ...
        var price = await GetCPrice(context, (int)request.CustomerId, unit.TrackingUnitModelId);
            ...
            case UStatus.Used:
                {
                    ...
                    serviceLog.Amount = await GetSPrice(context, ServiceTask.ReInstall);

                    unit.CustomerId = request.CustomerId;

                    if (unit.SimCardId != null && sim.Id != unit.SimCardId)
                    {
                        var oldSimCard = context.SimCards.Where(a => a.Id == (int)unit.SimCardId).First();
