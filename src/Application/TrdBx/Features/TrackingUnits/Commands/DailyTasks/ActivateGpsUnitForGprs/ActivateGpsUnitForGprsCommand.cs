...
public class ActivateTrackingUnitForGprsCommandHandler : SubscriptionSharedLogic, IRequestHandler<ActivateTrackingUnitForGprsCommand, Result>
{
    ...
    public async Task<Result> Handle(ActivateTrackingUnitForGprsCommand request, CancellationToken cancellationToken)
    {
        ...
        var unit = await cnx.TrackingUnits.Where(x => x.Id == request.Id).Include(x => x.Subscriptions).ThenInclude(s => s.ServiceLog).FirstAsync(cancellationToken) ?? throw new NotFoundException($"TrackingUnit with id: [{request.Id}] not found.");
        ...
        var result = await cnx.SaveChangesAsync(cancellationToken);
