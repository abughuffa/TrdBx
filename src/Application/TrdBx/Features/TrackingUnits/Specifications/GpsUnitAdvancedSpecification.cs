...
public class TrackingUnitAdvancedSpecification : Specification<TrackingUnit>
{
    public TrackingUnitAdvancedSpecification(TrackingUnitAdvancedFilter filter)
    {
        ...
        Query.Where(q => q.SNo != null)
             ...
             .Where(x => x.CreatedAt >= todayrange.Start && x.CreatedAt < todayrange.End.AddDays(1), filter.ListView == TrackingUnitListView.TODAY)
             .Where(x => x.CreatedAt >= last30daysrange.Start, filter.ListView == TrackingUnitListView.LAST_30_DAYS);
