using CleanArchitecture.Blazor.Application.TrdBx.Features.MyData.Local.Summaries.SimCards.Caching;
using CleanArchitecture.Blazor.Application.TrdBx.Features.MyData.Local.Summaries.SimCards.DTOs;
using CleanArchitecture.Blazor.Domain.Enums;


namespace CleanArchitecture.Blazor.Application.TrdBx.Features.MyData.Local.Summaries.SimCards.Queries.GetSummary;
     ...
     IRequestHandler<GetSimCardSummaryQuery, Result<SimCardSummaryDto>>
{
    ...
    public async ValueTask<Result<SimCardSummaryDto>> Handle(GetSimCardSummaryQuery request, CancellationToken cancellationToken)
    {
        await using var context = await _dbContextFactory.CreateAsync(cancellationToken);

        var data = await context.SimCards.GroupBy(x => 1) // Group all records together
