...
public class DataDiagnosisesWithPaginationQueryHandler : IRequestHandler<DataDiagnosisesWithPaginationQuery, PaginatedData<DataDiagnosisDto>>
{
    ...
    public async ValueTask<PaginatedData<DataDiagnosisDto>> Handle(DataDiagnosisesWithPaginationQuery request, CancellationToken cancellationToken)
    {
            ...
            case DataDiagnosisListView.SimCardsOfUnitsWhichAreExistOnTrdBxAndWialon:
                {
                    diagnostics = await (from l in _context.LibyanaSimCards
                                             ...
                                             select new DataDiagnosis
                                             {
                                              ...
                                              .ProjectToPaginatedDataAsync<DataDiagnosis, DataDiagnosisDto>(request.Specification,
                                                    request.PageNumber,
                                                    request.PageSize,
                                                    _typeAdapterConfig,
                                                    cancellationToken);
