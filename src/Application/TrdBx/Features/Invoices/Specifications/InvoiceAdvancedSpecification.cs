...
public class InvoiceAdvancedSpecification : Specification<Invoice>
{
    public InvoiceAdvancedSpecification(InvoiceAdvancedFilter filter)
    {
        ...
        Query.Where(q => q.InvNo != null)
             ...
             .Where(q => q.CreatedById == filter.CurrentUser.UserId, filter.ListView == InvoiceListView.My && filter.CurrentUser is not null)
             .Where(x => x.CreatedAt >= todayrange.Start && x.CreatedAt < todayrange.End.AddDays(1), filter.ListView == InvoiceListView.TODAY)
             .Where(x => x.CreatedAt >= last30daysrange.Start, filter.ListView == InvoiceListView.LAST_30_DAYS);
    }
}
