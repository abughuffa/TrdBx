...
#nullable disable warnings
    public async Task<Result<byte[]>> Handle(DownloadInvoiceByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        var invoice = await db.Invoices.Include(x => x.InvoiceItems)
        ...
        var serviceLogs = await db.ServiceLogs.Include(s => s.Subscriptions).Where(s => serviceLogIds.Contains(s.Id)).ToListAsync();
