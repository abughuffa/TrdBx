...

{
    ...
    //     _context = context;
    // }
    public async ValueTask<Result<int>> Handle(DeleteInvoiceCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            foreach (var itemGroup in invoice.InvoiceItemGroups)
            {
                context.InvoiceItems.RemoveRange(itemGroup.InvoiceItems); // Delete items first
            }
            context.InvoiceItemGroups.RemoveRange(invoice.InvoiceItemGroups); // Delete item groups


            invoice.AddDomainEvent(new InvoiceDeletedEvent(invoice));

            context.Invoices.Remove(invoice); // Delete invoice
