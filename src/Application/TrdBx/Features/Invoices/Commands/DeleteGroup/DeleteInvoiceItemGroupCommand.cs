...

{
    ...
    //     _context = context;
    // }
    public async ValueTask<Result<int>> Handle(DeleteInvoiceItemGroupCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var itemGroup = await context.InvoiceItemGroups
            ...
            context.InvoiceItems.RemoveRange(itemGroup.InvoiceItems);


            itemGroup.AddDomainEvent(new InvoiceItemGroupDeletedEvent(itemGroup));

            context.InvoiceItemGroups.Remove(itemGroup); // Remove the group itself

            // Get remaining groups in the invoice
            var remainingGroups = await context.InvoiceItemGroups
