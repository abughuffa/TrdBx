...
#nullable disable warnings
    public async ValueTask<Result<byte[]>> Handle(ExportTrackedAssetsQuery request, CancellationToken cancellationToken)
    {
        ...
        var data = await context.TrackedAssets.ApplySpecification(request.Specification)
              .OrderBy($"{request.OrderBy} {request.SortDirection}")
        ...
        var result = await _excelService.ExportAsync(data,
            ...
            {
                    {_localizer[_dto.GetMemberDisplayName(x=>x.TrackedAssetNo)],item => item.TrackedAssetNo},
                    {_localizer[_dto.GetMemberDisplayName(x=>x.TrackedAssetCode)],item => item.TrackedAssetCode},
