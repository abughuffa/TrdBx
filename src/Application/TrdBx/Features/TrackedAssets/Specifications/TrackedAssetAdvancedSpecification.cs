...
public class TrackedAssetAdvancedSpecification : Specification<TrackedAsset>
{
    public TrackedAssetAdvancedSpecification(TrackedAssetAdvancedFilter filter)
    {
        ...
        Query.Where(q => q.TrackedAssetNo != null)
             ...
             .Where(x => x.CreatedAt >= todayrange.Start && x.CreatedAt < todayrange.End.AddDays(1), filter.ListView == TrackedAssetListView.TODAY)
             .Where(x => x.CreatedAt >= last30daysrange.Start, filter.ListView == TrackedAssetListView.LAST_30_DAYS);
