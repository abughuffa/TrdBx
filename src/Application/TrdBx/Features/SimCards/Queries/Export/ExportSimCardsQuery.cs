...
#nullable disable warnings
    public async ValueTask<Result<byte[]>> Handle(ExportSimCardsQuery request, CancellationToken cancellationToken)
        {
        ...
        var data = await context.SimCards.ApplySpecification(request.Specification)
    .OrderBy($"{request.OrderBy} {request.SortDirection}")
...
{_localizer[_dto.GetMemberDisplayName(x=>x.ICCID)],item => item.ICCID},
{_localizer[_dto.GetMemberDisplayName(x=>x.SPackageId)],item => item.SPackageId},
