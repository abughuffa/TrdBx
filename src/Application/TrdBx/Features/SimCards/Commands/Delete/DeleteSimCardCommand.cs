...

{
    ...
    //     _context = context;
    // }
    public async ValueTask<Result> Handle(DeleteSimCardCommand request, CancellationToken cancellationToken)
    {

        await using var context = await _dbContextFactory.CreateAsync(cancellationToken);
        var items = await context.SimCards.Where(x => request.Id.Contains(x.Id)).ToListAsync(cancellationToken);
