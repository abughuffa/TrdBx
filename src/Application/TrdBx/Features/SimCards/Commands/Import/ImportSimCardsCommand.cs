             ...
             IRequestHandler<ImportSimCardsCommand, Result<int>>
{
    ...
    public async ValueTask<Result<int>> Handle(ImportSimCardsCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = await _excelService.ImportAsync(request.Data, mappers: new Dictionary<string, Func<DataRow, SimCardDto, object?>>
            {
                ...
                { _localizer[_dto.GetMemberDisplayName(x=>x.SPackageId)], (row, item) => item.SPackageId = int.Parse(row[_localizer[_dto.GetMemberDisplayName(x=>x.SPackageId)]].ToString()) },
                ...
                { _localizer[_dto.GetMemberDisplayName(x=>x.ICCID)], (row, item) => item.ICCID = row[_localizer[_dto.GetMemberDisplayName(x=>x.ICCID)]].ToString() },
                { _localizer[_dto.GetMemberDisplayName(x=>x.SStatus)], (row, item) => item.SStatus = (SStatus)Convert.ToInt32(row[_localizer[_dto.GetMemberDisplayName(x=>x.SStatus)]].ToString()) },
        ...
        if (result.Succeeded && result.Data is not null)
        {
            ...
            {
                var exists = await context.SimCards.AnyAsync(x => x.SimCardNo == dto.SimCardNo, cancellationToken);
                if (!exists)
                {
                    //var item = _mapper.Map<SimCard>(dto);
                    var item = _objectMapper.Map<SimCard>(dto);
                    // add create domain events if this entity implement the IHasDomainEvent interface
                    // item.AddDomainEvent(new ContactCreatedEvent(item));
                    await context.SimCards.AddAsync(item, cancellationToken);
                }
            }
    ...
    public async ValueTask<Result<byte[]>> Handle(CreateSimCardsTemplateCommand request, CancellationToken cancellationToken)
    {
        var fields = new string[] {
                            _localizer[_dto.GetMemberDisplayName(x=>x.Id)],
_localizer[_dto.GetMemberDisplayName(x=>x.SimCardNo)],
_localizer[_dto.GetMemberDisplayName(x=>x.ICCID)],
_localizer[_dto.GetMemberDisplayName(x=>x.SPackageId)],
