...
public class CusPriceAdvancedSpecification : Specification<CusPrice>
{
    public CusPriceAdvancedSpecification(CusPriceAdvancedFilter filter)
    {
        ...
        Query.Where(q => q.Price != 0.0m)
             ...
             .Where(x => x.CreatedAt >= todayrange.Start && x.CreatedAt < todayrange.End.AddDays(1), filter.ListView == CusPriceListView.TODAY)
             .Where(x => x.CreatedAt >= last30daysrange.Start, filter.ListView == CusPriceListView.LAST_30_DAYS);
