             ...
             IRequestHandler<ImportSPackagesCommand, Result<int>>
{
    ...
    public async ValueTask<Result<int>> Handle(ImportSPackagesCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = await _excelService.ImportAsync(request.Data, mappers: new Dictionary<string, Func<DataRow, SPackageDto, object?>>
            {
                { _localizer[_dto.GetMemberDisplayName(x=>x.Id)], (row, item) => item.Id = int.Parse(row[_localizer[_dto.GetMemberDisplayName(x=>x.Id)]].ToString()) },
                 { _localizer[_dto.GetMemberDisplayName(x=>x.SProviderId)], (row, item) => item.SProviderId = int.Parse(row[_localizer[_dto.GetMemberDisplayName(x=>x.SProviderId)]].ToString()) },
        ...
        if (result.Succeeded && result.Data is not null)
        {
            ...
            {
                var exists = await context.SPackages.AnyAsync(x => x.Name == dto.Name, cancellationToken);
                if (!exists)
                {
                    //var item = _mapper.Map<SPackage>(dto);

                    var item = _objectMapper.Map<SPackage>(dto);
                    // add create domain events if this entity implement the IHasDomainEvent interface
                    // item.AddDomainEvent(new ContactCreatedEvent(item));
                    await context.SPackages.AddAsync(item, cancellationToken);
                }
            }
    ...
    public async ValueTask<Result<byte[]>> Handle(CreateSPackagesTemplateCommand request, CancellationToken cancellationToken)
    {
        var fields = new string[] {
            
             _localizer[_dto.GetMemberDisplayName(x=>x.SProviderId)],
