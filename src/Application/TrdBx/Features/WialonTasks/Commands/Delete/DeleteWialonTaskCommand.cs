...

{
    ...
    public async ValueTask<Result> Handle(DeleteWialonTaskCommand request, CancellationToken cancellationToken)
    {

        await using var context = await _dbContextFactory.CreateAsync(cancellationToken);
        var items = await context.WialonTasks.Where(x => request.Id.Contains(x.Id)).ToListAsync(cancellationToken);
