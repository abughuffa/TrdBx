...

{
    ...
    public async Task<Result<int>> Handle(ExecuteWialonTaskCommand request, CancellationToken cancellationToken)
    {
        ...
        var item = await db.WialonTasks.Where(x => x.Id == request.Id).FirstOrDefaultAsync(cancellationToken);
        ...
        var oldItems = await db.WialonTasks.Where(x =>
        ...
        switch (item.APITask)
            {
                case APITask.AddToWialon:
                    {
                    var unit = db.TrackingUnits.Include(u => u.Customer.Parent)
                ...
                case APITask.ActivateOnWialon:
                    {
                    var unit = db.TrackingUnits.Where(u => u.Id == item.TrackingUnitId).FirstAsync().Result;
                    if (request.ApplyChangesToWialon)
                        {
                ...
                {
                    var unit = db.TrackingUnits.Include(u => u.SimCard).Where(u => u.Id == item.TrackingUnitId).FirstAsync().Result;
