...

{
    ...
    public async ValueTask<Result<int>> Handle(ExecuteWialonTaskCommand request, CancellationToken cancellationToken)
    {
        await using var context = await _dbContextFactory.CreateAsync(cancellationToken);

        var item = await context.WialonTasks.Where(x => x.Id == request.Id).FirstOrDefaultAsync(cancellationToken);
        ...
        var oldItems = await context.WialonTasks.Where(x =>
